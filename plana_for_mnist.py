...
input_count = 0  
for i in range(0,10):  
    dir = './mnist_digits_images_alltrain/%s/' % i                 # 这里可以改成你自己的图片目录，i为分类标签  
...
for i in range(0,10):  
    ...
    for rt, dirs, files in os.walk(dir):  
        for filename in files:  
            ...
            #以下内容为自己改的做压缩感知的部分
            input_images[index]=input_images[index].astype(np.float)
            #print(input_images[index])
            #print(x_test)
            #print(obmatrix)
            #obmatrix = 1 * np.random.randn(784,compression_nodenum) +0
            cinput[index]=np.matmul(input_images[index],obmatrix)
            #print(input_images[index])
...
input_count = 0  
for i in range(0,10):  
    dir = './mnist_digits_images_test/%s/' % i                 # 这里可以改成你自己的图片目录，i为分类标签  
    for rt, dirs, files in os.walk(dir):  
        for filename in files:  
            input_count += 1  
  
# 定义对应维数和各维长度的数组  
t_input_images = np.array([[0]*784 for i in range(input_count)])  
t_input_labels = np.array([[0]*10 for i in range(input_count)])  
t_cinput=np.array([[0]*compression_nodenum for i in range(input_count)],dtype=np.float)
        ...
        for filename in files:  
            ...
            for h in range(0, height):  
                for w in range(0, width):  
                    # 通过这样的处理，使数字的线条变细，有利于提高识别准确率  
                    if img.getpixel((w, h)) > 230:  
                        t_input_images[index][w+h*width] = 0  
                    else:  
                        t_input_images[index][w+h*width] = 1
            #以下内容为自己改的做压缩感知的部分
            t_input_images[index]=t_input_images[index].astype(np.float)
            #print(input_images[index])
            #print(x_test)
            #print(obmatrix)
            #obmatrix = 1 * np.random.randn(784,compression_nodenum) +0
            t_cinput[index]=np.matmul(t_input_images[index],obmatrix)
            #print(input_images[index])
            #print(np.matmul(x_test,obmatrix))
            #yuandaima           
            t_input_labels[index][i] = 1  
            index += 1  
...
# 全连接层  
W_fc1 = tf.Variable(tf.truncated_normal([3* 3 * 64, 1024], stddev=0.1))  
b_fc1 = tf.Variable(tf.constant(0.1, shape=[1024]))  
  
h_pool2_flat = tf.reshape(L2_pool, [-1, 3*3*64])  
...
with tf.Session() as sess:  
    ...
    batch_size = 60  
    iterations = 8000  
    batches_count = int(55000 / batch_size)  
    ...
    for it in range(iterations):  
        ...
        for n in range(batches_count):  
            train_step.run(feed_dict={x: cinput[n*batch_size:(n+1)*batch_size], y_: input_labels[n*batch_size:(n+1)*batch_size], keep_prob: 0.5})  
        if remainder > 0:  
            start_index = batches_count * batch_size;  
            train_step.run(feed_dict={x: cinput[start_index:input_count-1], y_: input_labels[start_index:input_count-1], keep_prob: 0.5})  
  
        # 每完成五次迭代，判断准确度是否已达到100%，达到则退出迭代循环  
        iterate_accuracy = 0  
        if it%50 == 0:  
            iterate_accuracy = accuracy.eval(feed_dict={x: t_cinput, y_: t_input_labels, keep_prob: 1.0})  
